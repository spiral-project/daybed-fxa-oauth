...
def includeme(config):
    ...
    if 'fxa-oauth.client_id' not in settings:
        raise ConfigurationError(
            'Missing fxa-oauth.client_id setting. '
            'This is needed to enable FxA OAuth'
        )

    if 'fxa-oauth.client_secret' not in settings:
        raise ConfigurationError(
            'Missing fxa-oauth.client_secret setting. '
            'This is needed to enable FxA OAuth'
        )

    client_id = settings.get("fxa-oauth.client_id")
    client_secret = settings.get("fxa-oauth.client_secret")

    oauth_uri = settings.get("fxa-oauth.oauth_uri",
                             "https://oauth.accounts.firefox.com/v1")
    content_uri = settings.get("fxa-oauth.content_uri",
                               "https://accounts.firefox.com")
    profile_uri = settings.get("fxa-oauth.profile_uri",
                               "https://profile.firefox.com/v1")
    ...
    config.registry['fxa-oauth.client_id'] = client_id
    config.registry['fxa-oauth.client_secret'] = client_secret
    ...
    config.scan("daybed_fxa_oauth.views")
